...
enable_interrupt_controller:
    mov w0,#PBASE
    mov w1,#ENABLE_BASIC_IRQS //0x3F00B218=Basic Interrupt enable Register
    add x1, x0, x1
...
*/
.equ ARM_TIMER_LOAD, 0x0000B400
.equ ARM_TIMER_CTRL, 0x0000B408

.globl timer_init_b
timer_init_b:
    mov w0,#PBASE
    mov w1,#ARM_TIMER_LOAD //0x3F00B400= timer count/load reagister
    add x1, x0, x1
 
    mov x2, #0x0A00000  // some delay count
    str x2, [x1]
  
     
    mov w0,#PBASE
    mov w1,#ARM_TIMER_CTRL //0x3F00B218=Basic Interrupt enable Register
    add x1, x0, x1
    
    mov x2, #0x00  // set the below flags
    ORR x2, x2, #(1 << 7) // timer enabled
    ORR x2, x2, #(1 << 5) // timer interrupt enabled
    ORR x2, x2, #(1 << 1) // 23 bit counter
    str x2, [x1]
